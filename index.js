...
async function run() {
    try {
        ...
        const bikeCollection = client.db('bike-house').collection('bikes');
        ...
        app.put('/bikes/:id', async (req, res) => {
            const id = req.params.id;
            const updatedBikes = req.body;
            const filter = { _id: ObjectId(id) };
            const options = { upsert: true };
            const updatedDoc = {
                $set: {
                    quantity: updatedBikes.quantity,
                    sold: updatedBikes.sold,
...
app.get('/', (req, res) => {
    res.send('Bike House server is running')
});

app.listen(port, () => {
    console.log('Bike House running on port 5000');
})